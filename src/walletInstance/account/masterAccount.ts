...
import {
  ...
  restoreKeyWalletFromBackupData,
} from '@src/services/key/keyWallet';
...
class MasterAccount extends BaseAccount implements MasterAccountInterface {
  ...
  static restoreFromBackupData(data: any, seed: Buffer) {
    ...
    account.child = child.map((accountData: any) =>
      Account.restoreFromBackupData(accountData)
    );
  ...
  getAccountByPrivateKey(privateKeySerialized: string) {
    ...
    return _.find(
      this.getAccounts(),
      (account) =>
        account.key.keySet.privateKeySerialized === privateKeySerialized
    );
  }
  ...
  async addAccount(
    ...
    try {
      ...
      const lastChildAccountIndex = _.findLastIndex(
        this.child,
        (account) => !account.isImport && !!account.key.childNumber
      );
      const lastChildAccount =
        lastChildAccountIndex !== -1 && this.child[lastChildAccountIndex];
      let newIndex = lastChildAccount
        ? new bn(lastChildAccount.key.childNumber).add(new bn(1)).toNumber()
        : 1;
      let keyData;
  ...
  removeAccount(name: string) {
    ...
    const removedAccounts = _.remove(
      this.child,
  ...
  async createAccountByPrivateKey(name: string, privateKey: string) {
    try {
      new Validator('name', name).required().string();
      new Validator('privateKey', privateKey).required().privateKey();
      L.info('Create account..', {
        name,
      ...
      if (type === 'PRIVATE_KEY') {
        ...
        const childAccountKeyWallet = new KeyWalletModel();
        const keySet = await getKeySetFromPrivateKeyBytes(
          (<PrivateKeyModel>privateKeyData.privateKey).privateKeyBytes
        );
        childAccountKeyWallet.chainCode = <Uint8Array>privateKeyData.chainCode;
        childAccountKeyWallet.childNumber = <Uint8Array>(
          privateKeyData.childNumber
        );
        ...
        throw new Error('Create account failed, private key is invalid');
      }
    } catch (e) {
      L.error('Create account failed', e);
      throw e;
    }
  }
  ...
  async importAccount(name: string, privateKey: string) {
    try {
      ...
      const account = await this.createAccountByPrivateKey(name, privateKey);
      this.child.push(account);
      return account;
    } catch (e) {
  ...
  async createAccountWithIndex({
    ...
    try {
      new Validator('name', name).required().string();
      new Validator('index', index).required().number();
      const keyData = await generateKey(this.seed, index, 0);
      const childAccountKeyWallet = new KeyWalletModel();
      childAccountKeyWallet.chainCode = keyData.chainCode;
      childAccountKeyWallet.childNumber = keyData.childNumber;
      childAccountKeyWallet.depth = keyData.depth;
